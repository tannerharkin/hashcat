...
// Reuse the DES S-box tables from m01500
...
DECLSPEC u32 generate_salt_byte (const u8 c)
{
  if (c >= 58 && c <= 64) return c + 7;   /* ':' - '@' maps to 'A' - 'G' */
  if (c >= 91 && c <= 96) return c + 6;   /* '[' - '`' maps to 'a' - 'f' */
  if (c >= 46 && c <= 122) return c;      /* '.' - 'z' unchanged */
  return '.';
}
...
DECLSPEC void _des_crypt_encrypt (PRIVATE_AS u32 *iv, const u32x salt, PRIVATE_AS u32x *Kc, PRIVATE_AS u32x *Kd, LOCAL_AS u32 (*s_SPtrans)[64])
{
  u32 r = 0;
  u32 l = 0;

  const u32 E1 = (salt >> 2) & 0x3f0;
  const u32 E0 = salt & 0x3f;

  // Unix crypt uses 25 iterations
  for (u32 i = 0; i < 25; i++) 
  {
    #ifdef _unroll
    #pragma unroll
...
KERNEL_FQ void m24000_mxx (KERN_ATTR_RULES ())
{
  ...
  /**
   * Precompute tables like in m01500
   */
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    /* Convert to DES key bits like in m01500 */
	data[0] = (tmp.i[0] << 1) & 0xfefefefe;
    data[1] = (tmp.i[1] << 1) & 0xfefefefe;

    /* Generate salt from password chars 2 and 3 */
    const u8 salt_char1 = (tmp.i[0] >> 8) & 0xff;
    const u8 salt_char2 = (tmp.i[0] >> 16) & 0xff;

    const u32 salt_byte1 = generate_salt_byte(salt_char1);
    const u32 salt_byte2 = generate_salt_byte(salt_char2);

    const u32 salt = ((u32)salt_byte1) | ((u32)salt_byte2 << 8);
    ...
    _des_crypt_encrypt (iv, salt, Kc, Kd, s_SPtrans);
...
KERNEL_FQ void m24000_sxx (KERN_ATTR_RULES ())
{
  const u64 gid = get_global_id (0);
  const u64 lid = get_local_id (0);
  const u64 lsz = get_local_size (0);
